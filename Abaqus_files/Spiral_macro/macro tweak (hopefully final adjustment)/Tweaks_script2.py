...
def BuildParametricSpiralFinal():
    ...
    # --- Loading Conditions ---
    applied_load_magnitude = 5000.0   # Surface traction force value
    # =====================================================================
    ...
    if 'BC-2' in mdb.models['Model-1'].boundaryConditions: del mdb.models['Model-1'].boundaryConditions['BC-2']
    ...
    for f in inst.faces:
        ...
        # Encastre ONLY on the faces matching Y=0 that are strictly inside the Tape Z-layer
        if abs(pt[1]) < 1e-5:
            if (z_gap_bottom - 1e-5) < pt[2] < (z_gap_top + 1e-5):
                encastre_faces.append(f)
                
        # Capture absolute top and absolute bottom surfaces for the load application
