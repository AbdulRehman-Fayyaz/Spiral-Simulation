...
def BuildParametricSpiralFinal():
    ...
    for i in range(num_quads - 1):
        ...
        # NEW: Bisecting partition right down the exact center
        pt1_M = (P1[0], P1[1], 0.0)
        pt2_M = (P2[0], P2[1], 0.0)
        pt3_M = (P1[0], P1[1], 1.0)
    ...
    # Track the edges belonging to Tape for independent meshing later
    ...
    # Global mesh seed for the wooden segments
    final_p.seedPart(size=(mesh_size_plywood_mm * scale), deviationFactor=0.1, minSizeFactor=0.1)
    
    # Specific edge seeds to create a fine mesh on the flexible tape
    tape_edge_seq = None
    ...
    # =====================================================================
    # 8. STEP & BOUNDARY CONDITIONS (Smart Face Selection)
    # =====================================================================
    a.regenerate()
    ...
    for f in inst.faces:
        ...
        # 1. Encastre on the Y=0 base face of the bottom layer
        if abs(pt[1]) < 1e-5 and pt[2] < z_gap_bottom:
            encastre_faces.append(f)
            
        # 2. Collect all faces belonging to the absolute Top or absolute Bottom Z-planes
        if abs(pt[2] - extrude_depth) < 1e-5:
            top_flat_faces.append(f)
        elif abs(pt[2] - 0.0) < 1e-5:
            bottom_flat_faces.append(f)

    # Apply Encastre Base Fix
    ...
    # Sort faces by Area (getSize) so the largest quad (Quad 0) is guaranteed to be Index 0
    if len(top_flat_faces) > 0 and len(bottom_flat_faces) > 0:
        ...
        # Apply -Z Traction on Bottom Face
