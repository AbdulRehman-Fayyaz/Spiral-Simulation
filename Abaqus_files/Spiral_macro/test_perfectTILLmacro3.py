...
def BuildParametricSpiralWithAllCuts():
    ...
    # --- Gap Cut Geometry (The Center Split) ---
    ...
    # --- Divider Cut Geometry (The Face Slots) ---
    divider_cut_width_mm = 0.5   # 0.5mm total width (0.25mm offset on both sides)
    divider_cut_depth_mm = 1.9   # Blind cut depth from the top and bottom faces
    # =====================================================================
    ...
    # 3. Calculate the Smart Gap Cut Polygon (Tool 1)
    ...
    for i in range(N_perim):
        if web_side == "LEFT" and i >= num_quads + 2:
            shifts.append(-web_thickness_mm * scale)
        elif web_side == "RIGHT" and 1 <= i <= num_quads:
            shifts.append(-web_thickness_mm * scale)
        else:
            shifts.append(2.0 * scale)
    ...
    # 4. Calculate the Face Divider Rectangles (Tool 2 & 3)
    divider_rects = []
    for i in range(num_quads - 1):
        # Find the shared boundary line (D to C of the current quad)
        P1 = quads[i][3] 
        P2 = quads[i][2] 
        dx, dy = P2[0] - P1[0], P2[1] - P1[1]
        L = math.hypot(dx, dy)
        ...
        # Build a rectangle around this line
        hw = (divider_cut_width_mm / 2.0) * scale
        ext = 5.0 * scale # Extend 5mm beyond the part to ensure a clean slice
    ...
    # =====================================================================
    # BUILD GEOMETRY & EXECUTE MULTIPLE ASSEMBLY CUTS
    # =====================================================================
    # Base Part
    ...
    for rect in divider_rects:
        for j in range(4):
            s3.Line(point1=rect[j], point2=rect[(j+1)%4])
    ...
    # Translate Tools to their correct Z-heights
    ...
    # Bottom divider stays at Z = 0.0
    
    # Execute a master subtraction generating the Final_Part
...
BuildParametricSpiralWithAllCuts()